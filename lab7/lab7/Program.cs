...
namespace SluthainiiDostupKSetiSinhronnoiAloha
{
    ...
    class Aloha
    {
        public void AlohaAsynchronous(int stationCount, int stationPacketCount, int stationPacketIntensity)
        {
            ...
            {
                // Создание станций и передача им начальных значений
            ...
            while (!AllPacketsSended(stations))
            {
                ...
                // если пакет данных был передан нужной станции без коллизий
                {
                    ...
                    LocalStation.packetCount--; // станция передаёт пакеты
                }
            ...
            Console.WriteLine("\nАлгоритм \"Алоха\" успешно завершил работу!");
            Console.WriteLine("Общее время работы алгоритма \"Алоха\": {0} ms.", globalTicks);
            Console.WriteLine("Общее количество отправленных пакетов алгоритма \"Алоха\": {0} шт.", (stationPacketCount * stationCount));
            Console.WriteLine("Общее число коллизий алгоритма \"Алоха\": {0} шт.", globalСollision);
        }


        public void AlohaSynchronous(int stationCount, int stationPacketCount, int stationPacketIntensity)
        {
            Random random = new Random(); // Генератор случайных чисел
            int globalTimeBetweenSendPacket = 1000 / stationPacketIntensity; // Временные промежутки между пакетами
            int globalTicks = 0;  // Затраченное время
            int globalСollision = 0; // Кол-во коллизий
            ...
            while (!AllPacketsSended(stations))
            {
                ...
                // если пакет данных был передан нужной станции без коллизий
                {
                    // Проходит такт времени
                    DecrementAllTimes(stations, ref globalTicks);
                    LocalStation.nextTime = globalTimeBetweenSendPacket; // обнуляем время передачи пакета так как он был передан
                    LocalStation.packetCount--; // станция передаёт пакеты
                }
                else if (stationReadyToSendCount > 1)
                // Если передача данных идет с коллизией
            ...
            Console.WriteLine("\nАлгоритм \"Синхронная Алоха\" успешно завершил работу!");
            Console.WriteLine("Общее время работы алгоритма \"Синхронная Алоха\": {0} ms.", globalTicks);
            Console.WriteLine("Общее количество отправленных пакетов алгоритма \"Синхронная Алоха\": {0} шт.", (stationPacketCount * stationCount));
            Console.WriteLine("Общее число коллизий алгоритма \"Синхронная Алоха\": {0} шт.", globalСollision);
        }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            aloha.AlohaAsynchronous(stationCount, stationPacketCount, stationPacketIntensity);
            aloha.AlohaSynchronous(stationCount, stationPacketCount, stationPacketIntensity);
            Console.ReadKey();
        }
    }
}